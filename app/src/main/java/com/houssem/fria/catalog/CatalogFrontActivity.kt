...
import com.houssem.fria.BoxMeasurements
...
class CatalogFrontActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        //setTheme(R.style.yellowBackground)
        super.onCreate(savedInstanceState)
        configureReceiver()

        setContentView(R.layout.categories_layout)
        appbar.setStatusBarForeground(
    ...
    fun openDepartmentPage(view: View, position: Int) {
        if (!isClicked) {
            ...
            val intent = Intent(this, DepartmentActivity::class.java).apply {
                putExtra("DEPARTMENT NAME", departmentAdapter.items[position].title)
            }
            ...
            var a=db.collection(departmentAdapter.items[position].departmentName).get()
