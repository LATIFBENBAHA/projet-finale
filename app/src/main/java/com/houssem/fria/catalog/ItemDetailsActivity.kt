...
import com.houssem.fria.ViewModel.FavoritesViewModel
import com.houssem.fria.ViewModel.FavoritesViewModelFactory
...
class ItemDetailsActivity : AppCompatActivity() {
    ...
    //var addFavoriteIcon: ImageButton()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*viewModal.favorites.observe(this){
            counter -> print("haha")
        }*/
        //
        ...
        var item = viewModal.favorites.value?.get(0)
        if(!(viewModal.any(selectedItem.id))){
            AddToFavoritesButton.setImageResource(R.drawable.ic_favorite_border)
            AddToFavoritesButton.setColorFilter(R.color.colorPrimary)
        }else{
            AddToFavoritesButton.setImageResource(R.drawable.ic_favorite_filled)
            AddToFavoritesButton.setColorFilter(R.color.colorPrimary)
        }
        ...
        AddToFavoritesButton.setOnClickListener {
            if(!(viewModal.any(selectedItem.id))){
                AddToFavoritesButton.setImageResource(R.drawable.ic_favorite_filled)
                AddToFavoritesButton.setColorFilter(R.color.colorPrimary)
                viewModal.insert(selectedItem)
            }else{
                AddToFavoritesButton.setImageResource(R.drawable.ic_favorite_border)
                AddToFavoritesButton.setColorFilter(R.color.colorPrimary)
                viewModal.delete(selectedItem)
    ...
    private fun loadSelectedItemImages() {
        ...
        for (s in selectedItem.images) {
            selectedItemImageList.add(SlideModel(s, ScaleTypes.FIT))
        }
    }
